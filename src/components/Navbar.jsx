      ...
      <div className="flex justify-between items-center px-container-padding py-6 max-w-[1440px] mx-auto w-full relative">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <div className="flex items-center gap-4 text-primary">
            ...
            <Link to={accountPath} className="hover:scale-110 transition-transform focus:outline-none" aria-label="User Account">
              <User className="h-5 w-5" />
            </Link>
            {user ? (
              <button 
                onClick={logout}
                className="ml-2 px-5 py-2 border border-primary hover:bg-primary hover:text-white rounded-full text-xs font-bold uppercase tracking-wider transition-all duration-300 focus:outline-none cursor-pointer"
              >
                Logout
              </button>
            ) : (
              <Link 
                to="/login"
                className="ml-2 px-5 py-2 bg-primary hover:bg-primary-container text-white rounded-full text-xs font-bold uppercase tracking-wider transition-all duration-300 block text-center"
              >
                Login
              </Link>
            )}
          ...
          <div className="pt-4 border-t border-outline-variant/20 mt-2">
            {user ? (
              <button 
                ...
                className="w-full py-3 bg-transparent border border-primary text-primary hover:bg-primary/5 rounded-full text-xs font-bold uppercase tracking-widest transition-colors focus:outline-none cursor-pointer"
              >
                Logout
              </button>
            ) : (
              <Link 
                to="/login" 
                onClick={() => setIsOpen(false)}
                className="w-full py-3 bg-primary hover:bg-primary-container text-white rounded-full text-xs font-bold uppercase tracking-widest block text-center transition-colors font-semibold"
              >
                Login
              </Link>
            )}
